...
/// Class to use so as to be able to deal with switch of dark and light modes in `UITableViewController` thanks to `DisplayModeNotifiable` protocol.
///
class CustomUITableViewController: UITableViewController, DisplayModeNotifiable, HasRootViewController {
    ...
    /// Using the `displayMode`,  updates the `barTintColor` and `tintColor` of the `navigationBar`defined in `customRootViewController``
